...
def get_env(new_key: str, old_key: str) -> str | None:
    ...
    if value is not None:
        return value
    return os.environ.get(old_key)


def server_url() -> str:
    return get_env("NETTRAINBRIDGE_SERVER_URL", "GRADMOTION_SERVER_URL") or DEFAULT_SERVER_URL


def auth_headers() -> dict[str, str]:
    token = get_env("NETTRAINBRIDGE_API_TOKEN", "GRADMOTION_API_TOKEN")
    if token:
