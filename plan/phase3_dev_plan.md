...
> **注意**：Web Dashboard（`static/index.html`、`dashboard.html`）已于去 GUI 化改造中删除。监控请使用 `cli/ntb.py`，详见 [remove_gui_plan.md](remove_gui_plan.md)。
...
- 通过 `train_with_metrics.py` 将 agi_origin 训练日志桥接为 NetTrainBridge 所需的 `metrics.jsonl`
...
### 1.1 仓库结构（与 NetTrainBridge 相关部分）
...
> **路径注意**：agi_origin README 写模型在 `log/`，Agent 默认搜索 `logs/**/model_*.pt`。阶段三需确认实际输出路径，必要时调整 `NETTRAINBRIDGE_MODEL_SEARCH_PATTERN` 或训练配置中的 `experiment_name`。
...
### 1.3 NetTrainBridge 与 agi_origin 的衔接缺口

| agi_origin 原生行为 | NetTrainBridge 需要 | 解决方案 |
|:---|:---|:---|
| 日志输出到 stdout | Agent 采集日志 → Dashboard SSE | ✅ 已有，无需改 |
| 不写 metrics.jsonl | Agent 读 `NETTRAINBRIDGE_METRICS_FILE` 上报曲线 | **train_with_metrics.py** |
...
train_with_metrics.py
    ...
    └─ 追加写入 NETTRAINBRIDGE_METRICS_FILE（metrics.jsonl）
            │
            ▼
...
**功能**:
- 读取环境变量 `NETTRAINBRIDGE_METRICS_FILE`（Agent 自动注入为 `{job_dir}/metrics.jsonl`）
...
export NETTRAINBRIDGE_CONDA_ENV=F1
...
# 若模型路径不匹配，按需调整：
# export NETTRAINBRIDGE_MODEL_SEARCH_PATTERN="log/**/model_*.pt"
```
...
**功能增强**:
- Webhook 签名校验（`NETTRAINBRIDGE_WEBHOOK_SECRET`）
- 仓库白名单（`NETTRAINBRIDGE_ALLOWED_REPOS`，仅允许 agi_origin）
...
export NETTRAINBRIDGE_WEBHOOK_SECRET=<your-secret>   # 可选
```
...
```bash
cd agi_origin
git commit --allow-empty -m "trigger nettrainbridge"
git push
...
export NETTRAINBRIDGE_METRICS_FILE=/tmp/metrics.jsonl
export NETTRAINBRIDGE_JOB_ID=test
...
git commit --allow-empty -m "nettrainbridge e2e test" && git push
...
| 模型路径 `log/` vs `logs/` 不匹配 | 调整 `NETTRAINBRIDGE_MODEL_SEARCH_PATTERN="log/**/model_*.pt"` |
